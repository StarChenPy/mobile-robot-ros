...
class Yolo5Lite:
    def __init__(self, model_pb_path, label_path, confThreshold=0.5, nmsThreshold=0.5):
        ...
        self.net = ort.InferenceSession(model_pb_path, so, providers=['AzureExecutionProvider', 'CPUExecutionProvider'])
    ...
    def letterBox(self, srcimg, keep_ratio=True):
        ...
        if keep_ratio and srcimg.shape[0] != srcimg.shape[1]:
            ...
            if hw_scale > 1:
                ...
                img = cv2.copyMakeBorder(img, 0, 0, left, self.input_shape[1] - neww - left, cv2.BORDER_CONSTANT,)  # add border
            else:
    ...
    def postprocess(self, frame, outs, pad_hw):
        ...
        predictions = []
        for ind in indices:
            frame = self.drawPred(frame, classIds[ind], confidences[ind], boxes[ind][0], boxes[ind][1], boxes[ind][2],
                                  boxes[ind][3])
            prediction = {
                'classId': classIds[ind],
                'confidence': confidences[ind],
                'box': boxes[ind]
            }
    ...
    def detect(self, srcimg):
        ...
        srcimg, predictions = self.postprocess(srcimg, outs, (newh, neww, top, left))
        ...
        return srcimg, predictions


if __name__ == '__main__':
    # 参数配置
    img_path = 'robotics/models/save1.jpg'
    ...
    # 加载模型
    net = Yolo5Lite(model_path, class_file, confThreshold, nmsThreshold)
    # 推理预测
    srcimg ,aa= net.detect(srcimg)
