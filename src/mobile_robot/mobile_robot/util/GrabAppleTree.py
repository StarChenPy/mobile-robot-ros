...
@singleton
class GrabAppleTree:
    def __init__(self, node, direction: Direction):
        self.logger = Logger()
        self.node = node
    ...
    def has_fruit(self):
        return bool(self.basket_1) or bool(self.basket_2) or bool(self.basket_3)
    ...
    def grab_apple_from_tree(self):
        ...
        for i in fruits:
            ...
            if fruit_type in self.basket_1:
                ArmMovement.put_fruit_to_basket(self.arm, 1)
                self.basket_1.remove(fruit_type)
            elif fruit_type in self.basket_2:
                ArmMovement.put_fruit_to_basket(self.arm, 2)
                self.basket_2.remove(fruit_type)
            elif fruit_type in self.basket_3:
                ArmMovement.put_fruit_to_basket(self.arm, 2)
                self.basket_3.remove(fruit_type)
